...
function checkAuth(apiKey) {
    ...
    var key = global.apiKeyDB.get({key: apiKey})
    if (!key) return false
    global.apiKeyDB.update(key,{lastUsedAt: new Date()})
    return key
}
...
module.exports = function(app) {
        ...
        req.on("end", function() {
            ...
            if (j.username == global.config.username) {
                var valid = speakeasy.totp.verify({ secret: global.config.totpSecret,
                    ...
                    token: j.totp })
                    if (valid) {
                        var k = randomString(32,'0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')
                        if (global.apiKeyDB.get({key: k})) {
                            res.status(500)
    ...
    app.get("/api/sessions", function(req,res) {
        ...
        if (authState && authState.accountManage) {
            var k = global.apiKeyDB.get({user: authState.user})
            if (!k.length) k = [k]
            var sk = []
            for (var key of k) {
                var isk = {}
